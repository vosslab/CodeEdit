...
/// UserDefaults key names backing the Settings scene (WP-F5) and the
...
/// `PlainEditor.fontSize` keys already written by the command-bar ribbon
/// (`CodeFileView.swift`); this type does not redeclare those two.
...
/// packages; WP-F5 only persists and exposes the setting.
...
/// document creation lands with the document-side packages; WP-F5 only
/// persists and exposes the setting.
